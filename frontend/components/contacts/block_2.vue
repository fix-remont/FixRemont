			...
			<UForm class="form" :state="state" :schema="FeedBackFormschema" @submit="onSubmit">
				...
				<UFormGroup class="checkbox-box" name="isAgree">
					<UCheckbox
						...
						:ui="{ base: 'w-[20px] h-[20px]  checked:bg-[var(--c-orange)] hover:checked:bg-[var(--c-orange)] focus:checked:bg-[var(--c-orange)]' }">
						<template #label>
							<span class="text">
								Согласен с условиями
								<a :class="['politic', 'hover']" :href="hrefSelfPolitic">Политики конфиденциальности</a>
							</span>
						</template>
					</UCheckbox>
				</UFormGroup>
